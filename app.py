...
def register():
    if request.method=='POST':
        ...
        if ccode==code:
            ...
            cursor.close()
            otp=genotp()
            subject='Thanls for registering to the application'
...
def home():
    if session.get('user'):        
        ...
        flash('Login before accessing the page')
...
def addnote():
    if session.get('user'):
        if request.method=='POST':
            ...
            mysql.connection.commit()
            cursor.close()
            flash(f'notes added successful')
...
def viewnotes (nid):
    ...
    return render_template('notesnew.html',data=data)
...
def viewfile(fid):
    if session.get('user'):
        cursor=mysql.connection.cursor()
        cursor.execute('select filename,filedata fron files where fid=%s',[fid])
        data=cursor.fetchone()
        cursor.close()
        filename=data[0]
        bin_file=data[1]
...
def forget():
    if request.method=='POST':
        ...
        if(rollno,) in data:
            ...
            flash('Reset link sent to ypur mail')
            return redirect(url_for('login'))
        else:
